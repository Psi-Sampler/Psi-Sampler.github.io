...
(B) shows the target distribution (red), defined by a reward that selects specific modes.

Each panel overlays:

🔴 Red dots: clean data samples obtained through SMC, where the proposal kernel is based on the denoising process of pre-trained score-based generative model.
...
**SMC Initialized with Prior Distribution:** *TDS* [[2](#ref-wu)] serves as the SMC baseline, while *DAS* [[3](#ref-kim)] extends it with a tempering strategy.  
**SMC Initialized with Posterior Distribution:** We compare four posterior-based initialization strategies: — *Top-K-of-N*, *Unadjusted Langevin Algorithm (ULA)*, *Metropolis-Adjusted Langevin
